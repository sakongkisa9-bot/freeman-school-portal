...
from cloud_service import CloudService, ask_cloud_credentials, apply_cloud_records_to_table

def calculate_junior_grade(score):
    """Junior School (CBC) Grading Logic"""
    try:
        score = int(score)
        if score >= 90: return "EE1", 8
        elif score >= 75: return "EE2", 7
        elif score >= 58: return "ME1", 6
        elif score >= 41: return "ME2", 5
        elif score >= 31: return "AE1", 4
        elif score >= 21: return "AE2", 3
        elif score >= 11: return "BE1", 2
        else: return "BE2", 1
    except ValueError:
        return "", 0
...
class JuniorMarkSheetView(ctk.CTkFrame):
    def __init__(self, parent, db_connection, class_name):
        ...
        try:
            ...
            if os.path.exists(json_path):
                with open(json_path, 'r') as f:
                    ...
                    self.current_exam_title = config.get("current_exam_title", "PERFORMANCE RECORD")
        except:
            pass
        ...
        self.canvas = ctk.CTkCanvas(self.canvas_container, bg="#242424", highlightthickness=0)
        ...
        self.table_inner_frame.bind("<Configure>", lambda e: self.canvas.configure(scrollregion=self.canvas.bbox("all")))
    ...
    def create_table_headers(self):
        ...
        ctk.CTkLabel(self.table_inner_frame, text="STUDENT NAME", font=("Arial Bold", 12), 
                     width=NAME_W, fg_color="gray25", corner_radius=0).grid(row=0, column=0, rowspan=2, sticky="nsew", padx=1, pady=1)

        for i, sub in enumerate(self.subjects):
            col_start = 1 + (i * 3)
            self.table_inner_frame.grid_columnconfigure((col_start, col_start+1, col_start+2), 
                                                        weight=0, minsize=BOX_SIZE, uniform="subs")
            
            # Subject Heading (Locked width)
            display_name = sub.upper()[:6] 
            ctk.CTkLabel(self.table_inner_frame, text=display_name, fg_color="gray30", corner_radius=0,
                         width=BOX_SIZE*3, font=("Arial Bold", 10)).grid(row=0, column=col_start, columnspan=3, sticky="nsew", padx=1, pady=(1, 0))
            
            # S, R, P Labels
            labels = [("S", "#1a1a1a"), ("R", "#2c3e50"), ("P", "#1e3a24")]
            for j, (txt, color) in enumerate(labels):
                ctk.CTkLabel(self.table_inner_frame, text=txt, fg_color=color, corner_radius=0, 
                             width=BOX_SIZE, height=BOX_SIZE, font=("Arial Bold", 10)).grid(row=1, column=col_start+j, sticky="nsew", padx=1, pady=1)

        # Totals
        total_start = 1 + (len(self.subjects) * 3)
        self.table_inner_frame.grid_columnconfigure((total_start, total_start+1, total_start+2), weight=0, minsize=TOT_W)

        for j, txt in enumerate(["TOT", "AVG", "RANK"]):
            ctk.CTkLabel(self.table_inner_frame, text=txt, font=("Arial Bold", 11), width=TOT_W,
                         fg_color="gray20", corner_radius=0).grid(row=0, column=total_start+j, rowspan=2, sticky="nsew", padx=1, pady=1)
    ...
    def add_student_row(self, student_data, row_index):
        ...
        ctk.CTkLabel(self.table_inner_frame, text=student_data[0], anchor="w", padx=10, 
                     width=NAME_W, font=("Arial", 12), fg_color="transparent").grid(row=grid_row, column=0, sticky="nsew", padx=1, pady=1)
        ...
        for i in range(1, (num_subjects * 3) + 1):
            ...
            e = ctk.CTkEntry(self.table_inner_frame, width=BOX_SIZE, height=BOX_SIZE, font=("Arial Bold", 12), 
                             fg_color=color, border_width=0, corner_radius=0, justify="center")
            ...
            if len(student_data) > i and student_data[i] is not None:
                ...
                if i % 3 != 1: e.configure(state="disabled")

            if i % 3 == 1:
                e.bind("<FocusOut>", lambda event, ent=e, col=i, r=grid_row: self.auto_fill_results(ent, col, r))
        ...
        for j in range(3):
            ...
            # Added width=65 here
            box = ctk.CTkEntry(self.table_inner_frame, width=65, height=BOX_SIZE, fg_color=summary_colors[j], 
                               font=("Arial Bold", 12), border_width=0, corner_radius=0, justify="center")
    ...
    def auto_fill_results(self, score_entry, col, row_index):
        ...
        try:
            r_entry = [w for w in self.table_inner_frame.grid_slaves(row=row_index, column=col+1)][0] 
            p_entry = [w for w in self.table_inner_frame.grid_slaves(row=row_index, column=col+2)][0]
    ...
    def update_totals(self, row_index):
        ...
        for sub_idx in range(1, num_subjects + 1):
            ...
            try:
                ...
                widgets = self.table_inner_frame.grid_slaves(row=row_index, column=p_col)
                if widgets:
                    val = widgets[0].get()
                    if val.isdigit():
        ...
        try:
            ...
            total_widgets = self.table_inner_frame.grid_slaves(row=row_index, column=total_box_col)
            if total_widgets:
            ...
            grades = [(7.5, "EE1"), (6.5, "EE2"), (5.5, "ME1"), (4.5, "ME2"),
                      (3.5, "AE1"), (2.5, "AE2"), (1.5, "BE1")]
            avg_grade = next((g for p, g in grades if mean_points >= p), "BE2")

            # Update Average Level Box (total_box_col + 1)
            avg_widgets = self.table_inner_frame.grid_slaves(row=row_index, column=total_box_col + 1)
            if avg_widgets:
    ...
    def create_bottom_controls(self):
        ...
        ctk.CTkButton(self.bottom_bar, text="💾 Save Marks", fg_color="#2e7d32", width=120, command=self.save_all_marks).pack(side="left", padx=5)
        ctk.CTkButton(self.bottom_bar, text="☁ Fetch from Cloud", fg_color="#1f538d", hover_color="#153d66", command=self.fetch_cloud_data).pack(side="left", padx=5)
        ctk.CTkButton(self.bottom_bar, text="🖨 Print PDF", fg_color="#f57c00", text_color="black", command=self.print_to_pdf).pack(side="left", padx=5)
        ctk.CTkButton(self.bottom_bar, text="🆕 New Exam", fg_color="#e67e22", command=self.create_new_exam).pack(side="left", padx=5)
        ctk.CTkButton(self.bottom_bar, text="🔄 Refresh", fg_color="#34495e", command=self.load_students_from_registry).pack(side="left", padx=5)
        ctk.CTkButton(self.bottom_bar, text="⬅ Back", fg_color="#942727", command=self.master.master.return_to_home).pack(side="right", padx=5)
    ...
    def fetch_cloud_data(self):
        ...
        if not result.get('success'):
            messagebox.showerror('Cloud Fetch Failed', result.get('message', 'Could not fetch cloud marks.'))
            return
        subjects = self.get_subjects_from_json()
        apply_cloud_records_to_table(self.table_inner_frame, result.get('records', []), subjects, columns_per_subject=3)
        messagebox.showinfo('Cloud Fetch', f"Fetched {len(result.get('records', []))} records from cloud.")

    def get_subjects_from_json(self):
        default_subjects = ["ENG", "KISW", "MATHS", "INT SCIE", "AGRI", "SST", "CRE", "C/A", "PRE-TECH"]
    ...
    def print_to_pdf(self):
        ...
        try:
            ...
            pdf.cell(0, 7, f"JUNIOR SCHOOL REPORT: {self.class_name.upper()} - {exam_name}", ln=True, align='C')
            pdf.ln(10) 
            ...
            for r in range(2, max_grid_row + 1):
                try:
                    ...
                    for i in range(1, (num_subs * 3) + 1):
                        cell_widgets = self.table_inner_frame.grid_slaves(row=r, column=i)
                        val = cell_widgets[0].get() if cell_widgets and hasattr(cell_widgets[0], 'get') else ""
                        pdf.cell(sub_col_w, 7, str(val), 1, 0, 'C')

                    total_col_idx = (num_subs * 3) + 1
                    for j in range(3):
                        t_widgets = self.table_inner_frame.grid_slaves(row=r, column=total_col_idx + j)
                        t_val = t_widgets[0].get() if t_widgets and hasattr(t_widgets[0], 'get') else ""
    ...
    def save_all_marks(self):
        ...
        for r in range(2, max_row + 1):
            try:
                ...
                self.db._cursor.execute("SELECT adm_no FROM students WHERE name = ?", (student_name,))
                ...
                for i, sub in enumerate(subjects):
                    ...
                    b = sub.replace(" ", "_").replace("-", "_").lower().replace("/", "_")
                    
                    # Get Score(s), Remark(r), and Points(p) from the grid cells
                    s_val = self.table_inner_frame.grid_slaves(row=r, column=col_start)[0].get() or "0"
                    r_val = self.table_inner_frame.grid_slaves(row=r, column=col_start+1)[0].get() or "BE2"
                    p_val = self.table_inner_frame.grid_slaves(row=r, column=col_start+2)[0].get() or "0"
                ...
                total_points = self.table_inner_frame.grid_slaves(row=r, column=total_col)[0].get() or "0"
                average_points = self.table_inner_frame.grid_slaves(row=r, column=total_col+1)[0].get() or ""
                ...
                self.db._cursor.execute("SELECT 1 FROM marksheet WHERE adm_no=?", (adm_no,))
                if not self.db._cursor.fetchone():
                    self.db._cursor.execute("INSERT INTO marksheet (adm_no) VALUES (?)", (adm_no,))
    ...
    def calculate_rankings(self):
        ...
        point_cols = [f"{s.replace(' ', '_').replace('-', '_').lower().replace('/', '_')}_p" for s in subjects]
        ...
        self.db._cursor.execute("SELECT adm_no FROM marksheet ORDER BY total_points DESC")
        ...
        for i, (adm,) in enumerate(ranked_students, start=1):
            self.db._cursor.execute("UPDATE marksheet SET rank = ? WHERE adm_no = ?", (i, adm))
    ...
    def create_new_exam(self):
        ...
        if new_title and messagebox.askyesno("Confirm", f"Reset marks for {self.class_name} and start '{new_title}'?"):
            # 1. Clear the marks in the DB
            self.db._cursor.execute("DELETE FROM marksheet WHERE adm_no IN (SELECT adm_no FROM students WHERE grade = ?)", (self.class_name,))
            self.db.conn.commit()
