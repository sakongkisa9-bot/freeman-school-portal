                ...
                if os.path.exists(path):
                    with open(path, "r") as f:
                        ...
                        self.current_exam_title = config.get(
                            "pp1_exam_title", "PP1 ASSESSMENT 2026"
                        )
        ...
        try:
            ...
            for sub in subjects:
                ...
                if s_col not in existing_cols:
                    ...
                    self.db._cursor.execute(
                        f"ALTER TABLE pp1_marks ADD COLUMN {s_col} INTEGER"
                    )
                ...
                if r_col not in existing_cols:
                    ...
                    self.db._cursor.execute(
                        f"ALTER TABLE pp1_marks ADD COLUMN {r_col} TEXT"
                    )

            # 4. Final check for Totals/Level
            if "total_points" not in existing_cols:
                self.db._cursor.execute(
                    "ALTER TABLE pp1_marks ADD COLUMN total_points INTEGER"
                )
            if "average_level" not in existing_cols:
                self.db._cursor.execute(
                    "ALTER TABLE pp1_marks ADD COLUMN average_level TEXT"
                )
        ...
        try:
            ...
            if self.read_only and self.marks_data:
                ...
                for row_data in records:
                    try:
                        score = int(row_data[total_idx]) if row_data[total_idx] is not None else -1
        ...
        if new_title:
            if messagebox.askyesno(
                ...
                try:
                    ...
                    # Clear DB marks
                    self.db._cursor.execute(
                        "DELETE FROM pp1_marks WHERE adm_no IN (SELECT adm_no FROM students WHERE grade = ?)",
                        (self.class_name,),
                    )
                    self.db.conn.commit()
