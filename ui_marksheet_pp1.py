...
class PP1MarkSheetView(ctk.CTkFrame):
    ...
    def __init__(self, parent, db_connection, class_name):
        ...
        self.current_exam_title = "PP1 ASSESSMENT 2026"
        try:
            import json
            import os
            ...
            if os.path.exists(path):
                with open(path, 'r') as f:
                    ...
                    self.current_exam_title = config.get("pp1_exam_title", "PP1 ASSESSMENT 2026")
    ...
    def get_subjects_from_json(self):
        ...
        try:
            ...
            with open(json_path, 'r') as f:
                ...
                # Navigate the nested dictionary: subjects -> pp1
                if "subjects" in config and "pp1" in config["subjects"]:
                    return config["subjects"]["pp1"]
                
                # Fallback for old flat JSON format
                if "pp1_subjects" in config:
                    return config["pp1_subjects"]
                    
        except Exception as e:
            print(f"Warning: Could not read JSON: {e}")
            
        # Default fallback if file is missing or broken
        return ["LANG", "MATH", "ENV", "PSYCH", "REL"]
    ...
    def create_bottom_controls(self):
        ...
        self.btn_save = ctk.CTkButton(self.bottom_bar, text="💾 Save Marks", fg_color="#2e7d32", hover_color="#1b5e20", width=120,command=self.save_pp1_marks)
    ...
    def create_table_headers(self):
        ...
        for i, sub in enumerate(subjects):
            col_start = 1 + (i * 2)
            h_frame.grid_columnconfigure((col_start, col_start+1), weight=0, minsize=24, uniform="primary_col")
    ...
    def add_student_row_with_data(self, row_data, rank):
        ...
        for i in range(num_subs * 2):
            col_idx = i + 1
            row_frame.grid_columnconfigure(col_idx, weight=0, minsize=24, uniform="primary_col")
            color = "#1a1a1a" if i % 2 == 0 else "#2c3e50"
            e = ctk.CTkEntry(row_frame, width=24, height=24, fg_color=color, border_width=1, corner_radius=0, justify="center")
    ...
    def ensure_table_exists(self):
        ...
        try:
            ...
            self.db._cursor.execute("CREATE TABLE IF NOT EXISTS pp1_marks (adm_no TEXT PRIMARY KEY)")
            
            # 2. Get currently existing columns
            self.db._cursor.execute("PRAGMA table_info(pp1_marks)")
            ...
            for sub in subjects:
                ...
                if s_col not in existing_cols:
                    print(f"DEBUG: Adding missing column [{s_col}] to pp1_marks...")
                    self.db._cursor.execute(f"ALTER TABLE pp1_marks ADD COLUMN {s_col} INTEGER")
                
                # Check Rate Column
                if r_col not in existing_cols:
                    print(f"DEBUG: Adding missing column [{r_col}] to pp1_marks...")
                    self.db._cursor.execute(f"ALTER TABLE pp1_marks ADD COLUMN {r_col} TEXT")
            
            # 4. Final check for Totals/Level
            if "total_points" not in existing_cols:
                self.db._cursor.execute("ALTER TABLE pp1_marks ADD COLUMN total_points INTEGER")
            if "average_level" not in existing_cols:
                self.db._cursor.execute("ALTER TABLE pp1_marks ADD COLUMN average_level TEXT")
        ...
        try:
            ...
            if os.path.exists(json_path):
                with open(json_path, 'r') as f:
                    ...
                    subjects = config.get("subjects", {}).get("pp1", [])
            
            if not subjects:
                subjects = ["LANG", "MATH", "ENV", "PSYCH", "REL"]
    ...
    def handle_new_exam(self):
        ...
        if new_title:
            if messagebox.askyesno("Confirm Reset", f"Clear marks and start {new_title}?"):
                try:
                    # Clear DB marks
                    self.db._cursor.execute("DELETE FROM pp1_marks WHERE adm_no IN (SELECT adm_no FROM students WHERE grade = ?)", (self.class_name,))
                    self.db.conn.commit()
                    ...
                    config["pp1_exam_title"] = self.current_exam_title
