...
class PrimaryMarkSheetView(ctk.CTkFrame):
        ...
        try:
            ...
            if os.path.exists(path):
                with open(path, 'r') as f:
                    ...
                    self.current_exam_title = config.get("primary_exam_title", "PRIMARY ASSESSMENT 2026")
    ...
    def get_subjects_from_json(self):
        ...
        try:
            ...
            with open(json_path, 'r') as f:
                ...
                # Navigate the nested dictionary: subjects -> primary
                if "subjects" in config and "primary" in config["subjects"]:
                    return config["subjects"]["primary"]
                
                # Fallback for old flat JSON format
                if "primary_subjects" in config:
                    return config["primary_subjects"]
                    
        except Exception as e:
            print(f"Warning: Could not read JSON: {e}")
            
        # Default fallback if file is missing or broken
        return ["ENG", "KISW", "MAT", "SCI", "AGR", "SST", "CRE", "C/A", "PHE"]
    ...
    def create_table_headers(self):
        ...
        ctk.CTkLabel(h_frame, text="STUDENT NAME", font=("Arial Bold", 11)).grid(row=0, column=0, rowspan=2)

        for i, sub in enumerate(subjects):
            col_start = 1 + (i * 2)
            h_frame.grid_columnconfigure((col_start, col_start+1), weight=0, minsize=45, uniform="primary_col")
            
            ctk.CTkLabel(h_frame, text=sub[:8], fg_color="gray30", font=("Arial Bold", 10)).grid(row=0, column=col_start, columnspan=2, sticky="nsew", padx=1, pady=1)
            ctk.CTkLabel(h_frame, text="Score", font=("Arial", 9), fg_color="#1a1a1a").grid(row=1, column=col_start, sticky="nsew")
            ctk.CTkLabel(h_frame, text="Rate", font=("Arial", 9), fg_color="#2c3e50").grid(row=1, column=col_start+1, sticky="nsew")

        t_idx = 1 + (num_subs * 2)
        h_frame.grid_columnconfigure((t_idx, t_idx+1, t_idx+2), weight=0, minsize=65, uniform="primary_col")
        ctk.CTkLabel(h_frame, text="TOTAL SCORES", font=("Arial Bold", 9)).grid(row=0, column=t_idx, rowspan=2, sticky="nsew")
        ctk.CTkLabel(h_frame, text="AVERAGE LEVEL", font=("Arial Bold", 9)).grid(row=0, column=t_idx+1, rowspan=2, sticky="nsew")
        ctk.CTkLabel(h_frame, text="POS", font=("Arial Bold", 9)).grid(row=0, column=t_idx+2, rowspan=2, sticky="nsew")
    ...
    def add_student_row_with_data(self, row_data, rank):
        ...
        ctk.CTkLabel(row_frame, text=row_data[0], anchor="w", padx=5, font=("Arial", 10)).grid(row=0, column=0)

        for i in range(num_subs * 2):
            col_idx = i + 1
            row_frame.grid_columnconfigure(col_idx, weight=0, minsize=45, uniform="primary_col")
            color = "#1a1a1a" if i % 2 == 0 else "#2c3e50"
            e = ctk.CTkEntry(row_frame, width=43, height=24, fg_color=color, border_width=1, corner_radius=0, justify="center")
        ...
        t_idx = 1 + (num_subs * 2)
        row_frame.grid_columnconfigure((t_idx, t_idx+1, t_idx+2), weight=0, minsize=65, uniform="primary_col")
        
        t_box = ctk.CTkEntry(row_frame, height=24, fg_color="gray30", corner_radius=0, justify="center")
        ...
        l_box = ctk.CTkEntry(row_frame, height=24, fg_color="#1f538d", corner_radius=0, justify="center")
        ...
        p_box = ctk.CTkEntry(row_frame, height=24, fg_color="#455a64", text_color="white", corner_radius=0, justify="center", font=("Arial Bold", 10))
    ...
    def ensure_table_exists(self):
        ...
        try:
            ...
            self.db._cursor.execute("CREATE TABLE IF NOT EXISTS primary_marks (adm_no TEXT PRIMARY KEY)")
            
            # 2. Get currently existing columns
            self.db._cursor.execute("PRAGMA table_info(primary_marks)")
            ...
            for sub in subjects:
                ...
                if s_col not in existing_cols:
                    print(f"DEBUG: Adding missing column [{s_col}] to primary_marks...")
                    self.db._cursor.execute(f"ALTER TABLE primary_marks ADD COLUMN {s_col} INTEGER")
                
                # Check Rate Column
                if r_col not in existing_cols:
                    print(f"DEBUG: Adding missing column [{r_col}] to primary_marks...")
                    self.db._cursor.execute(f"ALTER TABLE primary_marks ADD COLUMN {r_col} TEXT")
            
            # 4. Final check for Totals/Level
            if "total_points" not in existing_cols:
                self.db._cursor.execute("ALTER TABLE primary_marks ADD COLUMN total_points INTEGER")
            if "average_level" not in existing_cols:
                self.db._cursor.execute("ALTER TABLE primary_marks ADD COLUMN average_level TEXT")
    ...
    def save_primary_marks(self):
        ...
        try:
            for row_frame in self.table_inner.winfo_children():
                ...
                query = f"INSERT OR REPLACE INTO primary_marks ({', '.join(col_names)}) VALUES ({placeholders})"
        ...
        try:
            ...
            if os.path.exists(json_path):
                with open(json_path, 'r') as f:
                    ...
                    subjects = config.get("subjects", {}).get("primary", [])
            
            if not subjects:
                subjects = ["ENG", "KISW", "MAT", "SCI", "AGR", "SST", "CRE", "C/A", "PHE"]
    ...
    def handle_new_exam(self):
        ...
        if new_title:
            if messagebox.askyesno("Confirm Reset", f"Clear marks and start {new_title}?"):
                try:
                    # Clear DB marks
                    self.db._cursor.execute("DELETE FROM primary_marks WHERE adm_no IN (SELECT adm_no FROM students WHERE grade = ?)", (self.class_name,))
                    self.db.conn.commit()
                    ...
                    config["primary_exam_title"] = self.current_exam_title
