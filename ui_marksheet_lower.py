...
class LowerMarkSheetView(ctk.CTkFrame):
        ...
        try:
            ...
            if os.path.exists(path):
                with open(path, 'r') as f:
                    ...
                    self.current_exam_title = config.get("lower_exam_title", "LOWER PRIMARY ASSESSMENT 2026")
    ...
    def create_bottom_controls(self):
        ...
        self.btn_save = ctk.CTkButton(self.bottom_bar, text="💾 Save Marks", fg_color="#2e7d32", hover_color="#1b5e20", width=120,command=self.save_lower_marks)
    ...
    def ensure_table_exists(self):
        ...
        try:
            ...
            self.db._cursor.execute("CREATE TABLE IF NOT EXISTS lower_marks (adm_no TEXT PRIMARY KEY)")
            ...
            for sub in subjects:
                ...
                if s_col not in existing_cols:
                    ...
                    self.db._cursor.execute(f"ALTER TABLE lower_marks ADD COLUMN {s_col} INTEGER")
                
                # Check Rate Column
                if r_col not in existing_cols:
                    print(f"DEBUG: Adding missing column [{r_col}] to lower_marks...")
                    self.db._cursor.execute(f"ALTER TABLE lower_marks ADD COLUMN {r_col} TEXT")
            
            # 4. Final check for Totals/Level
            if "total_points" not in existing_cols:
                self.db._cursor.execute("ALTER TABLE lower_marks ADD COLUMN total_points INTEGER")
            if "average_level" not in existing_cols:
                self.db._cursor.execute("ALTER TABLE lower_marks ADD COLUMN average_level TEXT")
    ...
    def handle_new_exam(self):
        ...
        if new_title:
            if messagebox.askyesno("Confirm Reset", f"Clear marks and start {new_title}?"):
                try:
                    # Clear DB marks
                    self.db._cursor.execute("DELETE FROM lower_marks WHERE adm_no IN (SELECT adm_no FROM students WHERE grade = ?)", (self.class_name,))
                    self.db.conn.commit()
