...
class PP2MarkSheetView(ctk.CTkFrame):
    ...
    def __init__(self, parent, db_connection, class_name):
        ...
        self.current_exam_title = "PP2 ASSESSMENT 2026"
        try:
            import json
            import os
            ...
            if os.path.exists(path):
                with open(path, 'r') as f:
                    ...
                    self.current_exam_title = config.get("pp2_exam_title", "PP2 ASSESSMENT 2026")
    ...
    def get_subjects_from_json(self):
        ...
        try:
            ...
            with open(json_path, 'r') as f:
                ...
                # Navigate the nested dictionary: subjects -> pp2
                if "subjects" in config and "pp2" in config["subjects"]:
                    return config["subjects"]["pp2"]
                
                # Fallback for old flat JSON format
                if "pp2_subjects" in config:
                    return config["pp2_subjects"]
    ...
    def create_bottom_controls(self):
        ...
        self.btn_save = ctk.CTkButton(self.bottom_bar, text="💾 Save Marks", fg_color="#2e7d32", hover_color="#1b5e20", width=120,command=self.save_pp2_marks)
    ...
    def ensure_table_exists(self):
        ...
        try:
            ...
            self.db._cursor.execute("CREATE TABLE IF NOT EXISTS pp2_marks (adm_no TEXT PRIMARY KEY)")
            ...
            for sub in subjects:
                ...
                if s_col not in existing_cols:
                    ...
                    self.db._cursor.execute(f"ALTER TABLE pp2_marks ADD COLUMN {s_col} INTEGER")
                
                # Check Rate Column
                if r_col not in existing_cols:
                    print(f"DEBUG: Adding missing column [{r_col}] to pp2_marks...")
                    self.db._cursor.execute(f"ALTER TABLE pp2_marks ADD COLUMN {r_col} TEXT")
            
            # 4. Final check for Totals/Level
            if "total_points" not in existing_cols:
                self.db._cursor.execute("ALTER TABLE pp2_marks ADD COLUMN total_points INTEGER")
            if "average_level" not in existing_cols:
                self.db._cursor.execute("ALTER TABLE pp2_marks ADD COLUMN average_level TEXT")
        ...
        try:
            ...
            if os.path.exists(json_path):
                with open(json_path, 'r') as f:
                    ...
                    subjects = config.get("subjects", {}).get("pp2", [])
    ...
    def handle_new_exam(self):
        ...
        if new_title:
            if messagebox.askyesno("Confirm Reset", f"Clear marks and start {new_title}?"):
                try:
                    # Clear DB marks
                    self.db._cursor.execute("DELETE FROM pp2_marks WHERE adm_no IN (SELECT adm_no FROM students WHERE grade = ?)", (self.class_name,))
                    self.db.conn.commit()
                    ...
                    config["pp2_exam_title"] = self.current_exam_title
