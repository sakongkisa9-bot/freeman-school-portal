...
class PlaygroupMarkSheetView(ctk.CTkFrame):
    def get_json_path(self):
        ...
        return os.path.join(os.path.dirname(os.path.abspath(__file__)), 'school_config.json')
        ...
        try:
            ...
            if os.path.exists(path):
                with open(path, 'r') as f:
                    ...
                    self.current_exam_title = config.get("playgroup_exam_title", "PLAYGROUP ASSESSMENT 2026")
    ...
    def create_header_section(self):
        header_text = f"SHINNERS SCHOOL MATISI {self.class_name.upper()}\n2026 PERFORMANCE RECORD"
        # We store this as an instance variable 'self.header_label'
        self.header_label = ctk.CTkLabel(self, text=header_text, font=("Arial Bold", 22), 
                                         fg_color="#2e7d32", text_color="white", corner_radius=8, height=70)
    ...
    def update_header_display(self):
        ...
        display_text = f"{school_name}  |  {self.class_name.upper()}  |  {self.current_exam_title}"
    ...
    def create_bottom_controls(self):
        ...
        self.btn_save = ctk.CTkButton(self.bottom_bar, text="💾 Save Marks", fg_color="#2e7d32", hover_color="#1b5e20", width=120,command=self.save_primary_marks)
    ...
    def fetch_cloud_data(self):
        ...
        apply_cloud_records_to_table(self.table_inner, result.get('records', []), subjects, columns_per_subject=2)
    ...
    def create_scrollable_table_area(self):
        ...
        self.h_scroll = ctk.CTkScrollbar(self.container, orientation="horizontal", command=self.canvas.xview)
        self.v_scroll = ctk.CTkScrollbar(self.container, orientation="vertical", command=self.canvas.yview)
    ...
    def create_table_headers(self):
        ...
        h_frame.grid_columnconfigure(0, weight=0, minsize=STUDENT_NAME_WIDTH, uniform="primary_col")

        subjects = self.get_subjects_from_json()
        num_subs = len(subjects)

        ctk.CTkLabel(h_frame, text="STUDENT NAME", font=("Arial Bold", 11), width=STUDENT_NAME_WIDTH).grid(row=0, column=0, rowspan=2, sticky="nsew")

        for i, sub in enumerate(subjects):
            col_start = 1 + (i * 2)
            h_frame.grid_columnconfigure((col_start, col_start+1), weight=0, minsize=SUBJECT_COL_WIDTH, uniform="primary_col")
            
            ctk.CTkLabel(h_frame, text=sub[:8], fg_color="gray30", font=("Arial Bold", 10), width=SUBJECT_COL_WIDTH*2).grid(row=0, column=col_start, columnspan=2, sticky="nsew", padx=0, pady=0)
            ctk.CTkLabel(h_frame, text="Score", font=("Arial", 9), fg_color="#1a1a1a", width=SUBJECT_COL_WIDTH).grid(row=1, column=col_start, sticky="nsew", padx=0, pady=0)
            ctk.CTkLabel(h_frame, text="Rate", font=("Arial", 9), fg_color="#2c3e50", width=SUBJECT_COL_WIDTH).grid(row=1, column=col_start+1, sticky="nsew", padx=0, pady=0)

        t_idx = 1 + (num_subs * 2)
        h_frame.grid_columnconfigure((t_idx, t_idx+1, t_idx+2), weight=0, minsize=TOTAL_COL_WIDTH, uniform="primary_col")
        ctk.CTkLabel(h_frame, text="TOTAL SCORES", font=("Arial Bold", 9), width=TOTAL_COL_WIDTH).grid(row=0, column=t_idx, rowspan=2, sticky="nsew", padx=0, pady=0)
        ctk.CTkLabel(h_frame, text="AVERAGE LEVEL", font=("Arial Bold", 9), width=TOTAL_COL_WIDTH).grid(row=0, column=t_idx+1, rowspan=2, sticky="nsew", padx=0, pady=0)
        ctk.CTkLabel(h_frame, text="POS", font=("Arial Bold", 9), width=TOTAL_COL_WIDTH).grid(row=0, column=t_idx+2, rowspan=2, sticky="nsew", padx=0, pady=0)
    ...
    def add_student_row_with_data(self, row_data, rank):
        ...
        row_frame.grid_columnconfigure(0, weight=0, minsize=STUDENT_NAME_WIDTH, uniform="primary_col")
        ctk.CTkLabel(row_frame, text=row_data[0], anchor="w", padx=5, font=("Arial", 10), width=STUDENT_NAME_WIDTH).grid(row=0, column=0, sticky="nsew")

        for i in range(num_subs * 2):
            col_idx = i + 1
            row_frame.grid_columnconfigure(col_idx, weight=0, minsize=SUBJECT_COL_WIDTH, uniform="primary_col")
            color = "#1a1a1a" if i % 2 == 0 else "#2c3e50"
            e = ctk.CTkEntry(row_frame, width=SUBJECT_COL_WIDTH, height=24, fg_color=color, border_width=1, corner_radius=0, justify="center")
            ...
            if i % 2 == 0:
                e.bind("<FocusOut>", lambda event, ent=e, idx=i: self.auto_calculate(ent, idx, row_frame))
            else:
                e.configure(state="disabled")

        t_idx = 1 + (num_subs * 2)
        row_frame.grid_columnconfigure((t_idx, t_idx+1, t_idx+2), weight=0, minsize=TOTAL_COL_WIDTH, uniform="primary_col")
        
        t_box = ctk.CTkEntry(row_frame, width=TOTAL_COL_WIDTH, height=24, fg_color="gray30", corner_radius=0, justify="center")
        if row_data[t_idx] is not None: t_box.insert(0, str(row_data[t_idx]))
        t_box.configure(state="disabled")
        t_box.grid(row=0, column=t_idx, sticky="nsew", padx=0, pady=0)

        l_box = ctk.CTkEntry(row_frame, width=TOTAL_COL_WIDTH, height=24, fg_color="#1f538d", corner_radius=0, justify="center")
        if row_data[t_idx+1] is not None: l_box.insert(0, str(row_data[t_idx+1]))
        l_box.configure(state="disabled")
        l_box.grid(row=0, column=t_idx+1, sticky="nsew", padx=0, pady=0)

        p_box = ctk.CTkEntry(row_frame, width=TOTAL_COL_WIDTH, height=24, fg_color="#455a64", text_color="white", corner_radius=0, justify="center", font=("Arial Bold", 10))
        ...
        if val:
            try:
                ...
                if score > 100 or score < 0:
                    from tkinter import messagebox
                    messagebox.showwarning("Invalid Score", "Please enter a score between 0 and 100.")
    ...
    def ensure_table_exists(self):
        ...
        try:
            ...
            self.db._cursor.execute("CREATE TABLE IF NOT EXISTS playgroup_marks (adm_no TEXT PRIMARY KEY)")
            ...
            for sub in subjects:
                ...
                if s_col not in existing_cols:
                    print(f"DEBUG: Adding missing column [{s_col}] to playgroup_marks...")
                    self.db._cursor.execute(f"ALTER TABLE playgroup_marks ADD COLUMN {s_col} INTEGER")
                
                # Check Rate Column
                if r_col not in existing_cols:
                    print(f"DEBUG: Adding missing column [{r_col}] to playgroup_marks...")
                    self.db._cursor.execute(f"ALTER TABLE playgroup_marks ADD COLUMN {r_col} TEXT")
            
            # 4. Final check for Totals/Level
            if "total_points" not in existing_cols:
                self.db._cursor.execute("ALTER TABLE playgroup_marks ADD COLUMN total_points INTEGER")
            if "average_level" not in existing_cols:
                self.db._cursor.execute("ALTER TABLE playgroup_marks ADD COLUMN average_level TEXT")
    ...
    def save_primary_marks(self):
        ...
        try:
            for row_frame in self.table_inner.winfo_children():
                if not isinstance(row_frame, ctk.CTkFrame): continue
                ...
                if not widgets or widgets[0].cget("text") == "STUDENT NAME": continue
                ...
                col_names = ["adm_no"]
                for sub in subjects:
                    clean = sub.strip().replace(" ", "_").replace("-", "_").replace("/", "_").lower()
    ...
    def generate_pdf_report(self):
        ...
        try:
            ...
            pdf.cell(0, 8, f"GRADE: {self.class_name.upper()}  |  RECORD: {exam_name}", ln=True, align='C')
            pdf.ln(5)
    ...
    def handle_new_exam(self):
        ...
        dialog = ctk.CTkInputDialog(text="Enter New Exam Title:", title="New Performance Record")
        new_title = dialog.get_input()

        if new_title:
            if messagebox.askyesno("Confirm Reset", f"Clear marks and start {new_title}?"):
                try:
                    # Clear DB marks
                    self.db._cursor.execute("DELETE FROM playgroup_marks WHERE adm_no IN (SELECT adm_no FROM students WHERE grade = ?)", (self.class_name,))
                    self.db.conn.commit()
